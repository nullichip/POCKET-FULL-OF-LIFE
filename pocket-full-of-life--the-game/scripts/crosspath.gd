...
func _on_home_path_enter_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player"):
		body.velocity = Vector2.ZERO
		
		await TransitionScreen.transition_to_scene("", 0.0)
...
func _on_enter_body_entered(body: Node2D) -> void:
	if body.is_in_group("Player"):
		...
		#await TransitionScreen.transition_to_scene("whatever the park tscn is", 0.0)
